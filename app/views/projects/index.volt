...
<table class='table table-striped'>
	<thead><tr><th colspan='3'>{{model}}</th></tr></thead>
		...
		{% for object in objects %}
			<tr>
			<td>{{object}}</td>
			<td class='td-center'><a class='btn btn-primary btn-xs show' href='{{url.get(baseHref~"/show/"~object.getId())}}' data-ajax="{{ baseHref ~ "/show/" ~ object.getId() }}"><span class='glyphicon glyphicon-eye-open' aria-hidden='true'></span></a></td>
